...
function greedy_seam(energies, starting_pixel::Int)
    ...
    for i=2:m
		interval=energies[i, max(1, seam[i-1]-1):min(n, seam[i-1]+1)]
		seam[i]=findfirst(isequal(minimum(interval)), interval)+seam[i-1]-1-(seam[i-1]!=1)
	end
...
function least_energy(energies, i, j)
	...
	interval=energies[i+1, max(1, j-1):min(n, j+1)]
	c=j-1-(j!=1)
	possible_energies=[least_energy(energies, i+1, c+k)[1] for k=1:length(interval)]
	E_min=minimum(possible_energies)
	jₚ=findfirst(isequal(E_min), possible_energies)+c
    
	return (E_min+energies[i, j], jₚ)
end
...
Esse algoritmo testa as 2-3 escolhas possíveis a pertir de um determinado píxel, e recursivamente testa as seguintes 2-3 escolhas para cada escolha anterior, efetivamente fazendo uma "árvore" de testes que cobre todas as possibilidades a partir do píxel inicial. O é na ordem de (2|3)^(m-1) (onde 2|3→3 quando n→∞, devido ao efeito de bordas ser desprezível para n grande).
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	if !((i, j) ∈ keys(memory))
		...
		if i==m
			...
			c=j-1-(j!=1)
			possible_energies=[memoized_least_energy(energies, i+1, c+k, memory)[1] for k=1:3-(j==n||j==1)]
			E_min=minimum(possible_energies)
			jₚ=findfirst(isequal(E_min), possible_energies)+c
		    
			memory[(i, j)]=(E_min+energies[i, j], jₚ)
		end
	end
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	seam[1]=starting_pixel
	for i=1:m-1
    	seam[i+1]=memoized_least_energy(energies, i, seam[i], memory)[2]
	end
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
    if memory[i, j][1] < 0
		...
		if i==m
			...
			c=j-1-(j!=1)
			possible_energies=[matrix_memoized_least_energy(energies, i+1, c+k, memory)[1] for k=1:3-(j==n||j==1)]
			E_min=minimum(possible_energies)
			jₚ=findfirst(isequal(E_min), possible_energies)+c
...
function matrix_memoized_seam(energies, starting_pixel)
	m, n = size(energies)
	memory = fill((-1.0, -1), m, n)
	
    seam=Vector{Int64}(undef, m)
	seam[1]=starting_pixel
	for i=1:m-1
    	seam[i+1]=matrix_memoized_least_energy(energies, i, seam[i], memory)[2]
	end
...
function least_energy_matrix(energies)
    ...
    for i=m:-1:1
		for j=1:n
			result[i, j]=memoized_least_energy(energies, i, j, memory)[1]
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i=2:m
		interval=least_energies[i, max(1, seam[i-1]-1):min(n, seam[i-1]+1)]
		seam[i]=findfirst(isequal(minimum(interval)), interval)+seam[i-1]-1-(seam[i-1]!=1)
	end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "a6f975add7f91a526063e3cb0f6a373a64057a19"
