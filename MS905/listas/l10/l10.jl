...
Quando o $B$ é zero ou positivo, o fenômeno de aquecimento não sofre oposição ou é acentuado pelo feedback, respectivamente. A consequência é que a temperatura cresce sem limites, de forma linear (para $B=0$) ou supralinear ($B>0$).
"""
...
let
	plot(B->ECS(B=B), -30, 0, ylim=(0,5), label="ECS")
end
...
histogram(ECS_samples, label=nothing, xlabel="ECS", ylabel="samples", xlim=(-1, 20))
...
begin
    mean_ECS = sum(ECS_samples)/length(ECS_samples)
    ECS_mean_B = ECS(B=B̅) 
    prob_bigger = sum(([Int(i>ECS_mean_B) for i=ECS_samples]))/length(ECS_samples)
...
md"""
Se pautado somente no ECS do $\bar{B}$, teríamos uma impressão de que a temperatura aumentaria uma quantidade menor do que de fato estatisticamente o modelo prevê. Isso se dá pelo fato de que a função de ECS não preserva a média das amostras ($ECS(\bar{B})\neq \bar{ECS}(B)$), e de tal forma que a ECS média fica skewed (enviesado) para a direita, isto é, maior que o ECS da média. É obviamente recomendado fazer a média depois do cálculo do ECS de várias amostras para obter-se um resultado mais verossímil.
"""
...
simulated_rcp85_model, temperature_at_2100 = let
    ...
    Model.run!(ebm, 2100)
	plot(ebm.t, ebm.T, label="T(t) para RCP8.5", xlabel="t (anos)", ylabel="T (°C)"), ebm.T[end]
end

# ╔═╡ 869a426c-1b3b-4eb2-96f2-5a49b199ace1
...
expected_double_CO2_year = t[1]-1+findfirst(x->x>=560, Model.CO2_RCP85.(t))
...
prob_rcp26, prob_rcp85 = let
	RCP26=temperature_response.(Model.CO2_RCP26, B_samples)
	RCP85=temperature_response.(Model.CO2_RCP85, B_samples)

	p85=sum(([Int(i>Model.T0+2) for i=RCP85]))/length(RCP85)
	p26=sum(([Int(i>Model.T0+2) for i=RCP26]))/length(RCP26)
    p26, p85  
end
...
md"""
``log(CO2) = `` $(@bind log_CO2 Slider(log(10, CO2min):.001:log(10, CO2max); show_value=true, default=log(10, CO2min)))
"""
...
# ╔═╡ 909cb892-187e-4512-9c4f-a589d60951bf
...
co2_to_melt_snowball = let
	log_CO2=log(10, CO2min)
		
	while equilibrium_temperature(10^log_CO2)<-10 #-10 vem da condição de albedo
		log_CO2+=0.01
	end
