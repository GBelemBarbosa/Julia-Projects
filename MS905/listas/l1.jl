...
# ╔═╡ 5496577b-d25e-49ef-ab70-355436450eb4
a_rand = random_vect()

# ╔═╡ 2a391708-edee-11ea-124e-d14698171b68
m = mean(a_rand) # substitua `missing` com seu código!
...
create_bar() = vcat(zeros(Int8, 40), fill(1, 20), zeros(Int8, 40))
...
get_red(pixel::AbstractRGB) = pixel.r
...
get_reds(image::AbstractMatrix) = get_red.(image)
...
# ╔═╡ a545e6cf-2893-4738-a523-cfd047d5a034
get_greens(image::AbstractMatrix) = get_green.(image)

# ╔═╡ 2d910e1b-955c-4dc4-afd7-69b61ff674f4
get_blue(pixel::AbstractRGB) = pixel.b

# ╔═╡ ca7a6b49-e95e-4eaa-a5c0-51a182ba520a
...
mean_color(image) = RGB(mean(get_reds(image)), mean(get_greens(image)), mean(get_blues(image)))
...
quantize(x::Number) = floor(10*x)/10
...
quantize(color::AbstractRGB) = RGB(quantize(color.r), quantize(color.g), quantize(color.b))
...
noisify(color::AbstractRGB, s) = RGB(noisify(color.r, s), noisify(color.g, s), noisify(color.b, s))
...
A imagem se torna irreconhecível (para mim) com uma intensidade de ruído em torno de 2. O ruído ser maior que 1 não é surpreendente, pois $s$ delimita o valor (em módulo) máximo que será somado a cada píxel, mas o elemento randômico do método implica que para valores pequenos de $s$ boa parte dos valores que de fato serão somados são pequeno (em módulo); somente com $s$ maior, de fato, se torna improvável que o valor somado seja pequeno, desconfigurando a imagem mais eficientemente.
"""
...
#Filtro que retorna um pixel com somente os valores máximos entre r, g e b em suas respectivas posições (levando em consideração empates)
function custom_filter(pixel::AbstractRGB)
    r_value = pixel.r 
	g_value = pixel.g 
	b_value = pixel.b 
	dominant_value=max(r_value, g_value, b_value)

    return RGB((dominant_value==r_value)*r_value, (dominant_value==g_value)*g_value, (dominant_value==b_value)*b_value)
end

# ╔═╡ 9e5a08dd-332a-486b-94ab-15c49e72e522
custom_filter(image::AbstractMatrix) = custom_filter.(image)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "962178b5ace3828e3570b66e797dd6a1a957a308"
