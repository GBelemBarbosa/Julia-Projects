...
md"Com $p_{infection}$ fixo em 0.2, $p_{recovery}$ teve de ser colocado acima de 0.154 (usando só 3 casas decimais para a escolha)."
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	T = length(first(simulations).S)
	S, I, R=zeros(T), zeros(T), zeros(T)
	σS, σI, σR=zeros(T), zeros(T), zeros(T)
	num_simulations=length(simulations)
	...
	σS=.√(σS/num_simulations)
	σI=.√(σI/num_simulations)
	σR=.√(σR/num_simulations)
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i=1:T
		...
		for a=agents
			if is_susceptible(a)
				@inbounds S_counts[i]+=1
			elseif is_infected(a)
				@inbounds I_counts[i]+=1
			else
				@inbounds R_counts[i]+=1
			end
		end
	end
...
let
	Random.seed!(0)
	simulations2 = repeat_simulations(100, 1000, InfectionRecovery(0.2, 0.155), 200)
