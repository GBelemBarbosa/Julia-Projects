...
student = (name = "Gabriel Belém Barbosa", email_dac = "g234672")
...
ks, vs=collect(keys(test_counts)), collect(values(test_counts))
...
function counts2(data::Vector)
	dic=counts(data)
	ks, vs=collect(keys(dic)), collect(values(dic))
	perm=sortperm(ks)
...
function geometric(p::Real)
	if p==0 || p>1
		throw(ArgumentError("p não aceitável (p=0 ou p>1)"))
	else
		n=1
...
A função bernoulli retorna sempre True, e geometric retorna 1 (evento com probabilidade p sempre acontece na primeira iteração).
"""
...
experiment(p::Real, N::Integer)=[geometric(p) for i=1:N]
...
mean_fail_time = sum(large_experiment)/length(large_experiment)
...
md"
Foi possível observar que quanto maior o p, menor é a 'cauda' da curva e maior (menos valores são atingidos pela série geométrica) e mais concentrados estão os valores em 1. Isso sugere que a relação depende (1-p)^(i-1), sendo i a variável do eixo x. Contudo, analisando como a curva se comporta com p decrescente (com a barra de i=1 seguindo sempre bem próxima do próprio p) é revelada uma necessidade por uma dependência por p. A teoria modela essa curva como sendo p(1-p)^(i-1), o que condiz com os resultados observados, principalmente no gráfico com t em escala logarítima, no qual o comportamento se torna parecido com uma reta.
"
...
function t_falha(p::Real)
	exp=experiment(p, 10000)
	
	return sum(exp)/length(exp)
end
...
let
	p=[i for i=0.01:0.01:1]
		
	bar(p, [1 ./t_falha.(p)], alpha=0.5, leg=false)	
end

# ╔═╡ d2e4185e-8c51-11eb-3c31-637902456634
md"""
Baseado em minhas observações (bastante evidente pela forma semelhante à y'=x do gráfico no qual y'=1/y), parece que tem-se a seguinte relação:
...
Ps = let 
	...
	[p*(1-p)^i for i=0:49] # your code here
end
...
sum_Ps = sum(Ps)  # Add your code here
...
```math
\sum_{k=1}^{\infty} P_k = p\sum_{k=1}^{\infty}(1-p)^{k-1} = p\frac{1}{1-(1-p)}= 1

```
Onde foi usada a propriedade da soma da série geométrica infinita (1-p<1).
"""
...
let
	...
	bar(xs, ps, alpha=0.3, label="Amostral")	

	xs=[i for i=1:xs[end]] 
	ps=[p*(1-p)^i for i=0:xs[end]-1]
		
	bar!(xs, ps, alpha=0.3, label="P_n")	
end
...
md"Uma das formas é somar a diferença ao quadrado entre os valores de $P_n$ dos dois gráficos (par a par). Isso pode ser feito até o maior valor alcançado pela amostra e depois pode ser somado o quadrado dos elementos da cauda infinita da série geométrica (que pode ser obtida de forma analítica). Creio que essa medida daria a ideia da variânciacia dos dados amostrais.  
"
...
# ╔═╡ 7372a2c6-765e-4248-a5a7-6ba137198f4c
begin
	plot(
        vcat([0], cumulative), zeros(length(cumulative)+1),
        markershapes = :vline,
        linecolour = :black,
        markercolour = :black,
        markersizes = 9,
	...
	scatter!(rand(10), zeros(10), color="blue", label="Amostra")
end
...
md"""
```math
C_n := \sum_{k=1}^n P_k = p\sum_{k=1}^n (1-p)^{k-1}=p\frac{1-(1-p)^n}{p}=1-(1-p)^n
```
"""
...
# ╔═╡ 491dd4cc-1bdc-4a6d-87cf-86c0b9ee6c84
md"""
```math
r \geq Cn  \Rightarrow 1-r\leq(1-p)^n\Rightarrow log(1-r)\leq nlog(1-p)
```
"""
...
md"""
```math
\Rightarrow n(r,p) = \lfloor \frac{log(1-r)}{log(1-p)}\rfloor
```
"""
...
geometric_bin(u::Real, p::Real)=floor(log(1-u)/log(1-p))
...
fast_geometric_sample = [geometric_fast(10^(-10)) for i=1:10000] # Substitua com o seu código

# ╔═╡ c37bbb1f-8f5e-4097-9104-43ef65aa1cbd
let
	xs, ps = probability_distribution(fast_geometric_sample)
...
# ╔═╡ 588c078a-6e77-4287-99d9-ad6475a30b8d
N_1=10^4+1

# ╔═╡ 7d4f6eca-0264-4199-b287-93009d7758fa
p_a=0.55

# ╔═╡ 5e2b2f8b-a55b-4c8c-8b0f-4d57f4782edf
y0=10

# ╔═╡ deb5fbfb-1e03-42ce-a6d6-c8d3edd89a9a
function evolve(s, p)
	s′=similar(s)   # make a vector of the same length and type
	                  # to store the probability vector at the next time step
	
	for i in 2:length(s)-1
		s′[i]=(1-p)*s[i-1]+p*s[i+1]
	end
	
	s′[1]=p*s[1]+p*s[2] #prob de 1 é calculada diferente
	s′[end]=(1-p)*s[end-1]
...
# ╔═╡ 5f55af5f-5d32-44d3-911d-fa305f6b90b5
begin
	s=zeros(y0+N_1-1) #todos possíveis valores de y para N_1-1 passos
	s[y0]=1 #partícula começa em y0

	for i in 1:N_1
		s=evolve(s, p_a)
	end
	
	plot(s, ylim=(0, maximum(s)+0.05), xlim=(1, 100), leg=false, size=(500, 300))
end

# ╔═╡ 3bed43ab-9871-494b-9d83-3a3b53b056a9
let
	percurso=atmosphere(p_a, y0, 10^7)
	xs, ps=probability_distribution(percurso)
...
md"
Na implementação 'teórica' não foram usados 10^7 passos por motivos de tempo de processamento, porém mesmo assim a tendência da distribuição fica bem clara, sendo ela semelhante à distribuição geométrica estudada (comportamento exponencial relacionado à 1-p). O mesmo ocorre com a implementação simulada.
"

# ╔═╡ ddf49067-d9b5-4866-90d0-317fb4fb5431
md"
Obs. A escala no gráfico foi escolhida como sendo de 1 a 100 para melhor visualizar o comportamento da distribuição, visto que esta se concentra no início dos possíveis valores de altura.
"
...
# ╔═╡ 7a9027b0-afd8-4735-a409-c44eb5893348
N_2=10001 #número de passos +1 (estado inicial incluso)

# ╔═╡ 8517f92b-d4d3-46b5-9b9a-e609175b6481
begin
	s0=zeros(y0+N_2-1) #todos possíveis valores de y para N_2-1 passos
	s0[y0]=1 #partícula começa em y0
end
...
# ╔═╡ b465900e-5552-4a66-bbed-55e7d6201f51
md"""
t = $(@bind t Slider(1:N_2, show_value=true, default=1))
"""

# ╔═╡ 4f029f70-9515-4c0e-acc0-b91295e04618
plot(ss[t], ylim=(0, 1), xlim=(1, 100), leg=false, size=(500, 300))

# ╔═╡ cd9bdbf2-3c62-4f9c-aa35-c5afbc722940
sim=atmosphere(p_a, y0, N_2)

# ╔═╡ 317fffa3-a6a1-4e3e-bb7f-33357519f9f1
begin
	xsps=[probability_distribution(sim[1:1])]

	for i in 2:N_2
		append!(xsps, [probability_distribution(sim[1:i])])
	end
end

# ╔═╡ a1586c3d-6201-4c13-b1c9-3754e3b3c176
md"""
t = $(@bind tt Slider(1:N_2, show_value=true, default=1))
"""

# ╔═╡ 9249f928-3d39-4710-85bd-673cef508f61
bar(xsps[tt][1], xsps[tt][2], alpha=0.5, leg=false)

# ╔═╡ d3bec73d-0106-496d-93ae-e1e26534b8c7
md"
Quanto maior o número de passos, mais a distribuição teórica se parece contínua (sem os dentes característicos desse tipo de evolução) devido à assincronicidade causada por partículas no 'chão'. O efeito que o chão tem na distribuição é de ir amortecendo os dentes mais próximos gradativamente, com o efeito se espalhando para toda a distribuição quanto mais o passos. A distribuição vai se aproximando do comportamento de uma geométrica (mesmo que com p distinto). A distribuição simulada fica cada vez mais parecida com a teórica devido ao alto número de 'amostras' (apesar de não serem tecnicamente amostras, pois isso envolveria simular múltiplas partículas e pegar suas posições ao fim dos passos para montar o gráfico, contar quantas vezes uma partícula única passa por uma posição durante todo o passeio é suficientemente parecido para N grande), que diminui sua variância.
"
...
md"
Sim, pois a pressão atmosférica é em função da quantidade de partículas (coluna de ar) acima da medição, e o nosso modelo para comportamento atmosférico previu que essa quantidade de partículas não é constante nem cresce ou decresce linearmente (o que causariam curvas de pressões por altura lineares e quadráticas, respectivamente); de certa forma, o gráfico da pressão pode ser usado de forma intercambiável ao da densidade de partículas (no que diz respeito à sua forma geral e comportamento).
"
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "fe257dc044ed8d9c9031f63bd3c5755b81dccd78"
