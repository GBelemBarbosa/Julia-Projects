...
trajectory(w::Coordinate, n::Int)=accumulate(+, rand(possible_moves, n), init=w)
...
collide_boundary(c::Coordinate, L::Number)=Coordinate(max(min(c.x, L), -L), max(min(c.y, L), -L))
...
trajectory(w::Coordinate, n::Int, L::Number)=accumulate((a, b)->collide_boundary(a+b, L), rand(possible_moves, n), init=w)
...
let
	...
	for i=1:10
		plot_trajectory!(p, trajectory(Coordinate(0,0), 1000))      # plot one trajectory
	end

	p
end

# ╔═╡ 87b42221-5128-4a00-bc8a-6c67630c89f7
let
	long_trajectory = trajectory(Coordinate(4,4), 1000, 6)
...
visualize(agents::Vector, L)=scatter(ratio=1, xlim=(-L, L), ylim=(-L, L), make_tuple.(position.(agents)), c=color.(agents), label="")
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	if agent.status==S && agent.position==source.position && source.status==I && rand()<infection.p_infection
		agent.status=I
...
# ╔═╡ c0c34d86-9850-40bc-8f8b-df87ba68e26f
...
# ╔═╡ 23d53244-97c8-4b8a-9e9a-995a25cb3bbf
Int(Agent(Coordinate(0, 0), S).status)+1
...
let
	...
	for i=1:k_sweep_max
		...
		SIR=[0, 0, 0]
		for j=1:N
			@inbounds SIR[Int(agents[j].status)+1]+=1
		end
		push!(S, SIR[1])
		push!(I, SIR[2])
		push!(R, SIR[3])
...
# ╔═╡ 1836b2b4-c54c-4504-a309-f03e73df3b00
L=30
...
# ╔═╡ 447aff17-2337-4675-b986-6ab7874394b2
md"`t` = $(@bind t Slider(1:k_sweep_max, default=1, show_value = true))."

# ╔═╡ d55539bc-5cae-4c8d-b914-dbc626548108
let
	visualize(states[t], L)
end

# ╔═╡ 9cf0aa18-c92d-424a-b104-b8989c199e41
let
	N = length(states[1])
	Ss, Is, Rs = [N-1], [1], [0]
	
	@gif for t=1:50:k_sweep_max
		left = visualize(states[t], L)
		SIR=[0, 0, 0]
		for j=1:N
			@inbounds SIR[Int(states[t][j].status)+1]+=1
		end
		push!(Ss, SIR[1])
		push!(Is, SIR[2])
		push!(Rs, SIR[3])
	
		right = plot(xlim=(1, k_sweep_max/50), ylim=(1,N), size=(600,300))
...
# ╔═╡ 407a4804-08f1-4b13-b425-a42354dd0ac1
let
	N = 100
	L = 20
	agents=initialize(N, L)
	...
	plot_outbreak = visualize(agents, L)
	plot!(plot_outbreak, title="Causes outbreak")

	for i=1:k_sweep_max
		sweep!(agents2, L, does_not_cause_outbreak)
	end
	plot_not_outbreak = visualize(agents2, L)
	plot!(plot_not_outbreak, title="Does not cause outbreak")
	scatter!(plot_not_outbreak, [2*L, 2*L], label="S", c="blue")
	scatter!(plot_not_outbreak, [2*L, 2*L], label="I", c="red")
	scatter!(plot_not_outbreak, [2*L, 2*L], label="R", c="green")

	plot(plot_outbreak, plot_not_outbreak)
end

# ╔═╡ f6b9af9a-0942-4285-85ea-40a374cb421c
md"Como visto acima, para uma mesma taxa de infeccção de 0.5, p_recovery deve ser maior que 0.00001 para que não ocorrea outbreak nesse cenário (L=20, N=100)."
...
let
    N = 50
    L = 30
	P=[plot(title="Causes outbreak", ylim=(0, N)), plot(title="Does not cause outbreak", ylim=(0, N))]
	plot!(P[2], [0, 0], label="S", c="blue")
	plot!(P[2], [0, 0], label="I", c="red")
	plot!(P[2], [0, 0], label="R", c="green")
	epidemics=[causes_outbreak, does_not_cause_outbreak]
	for e=1:2
		for k=1:50
		    agents=initialize(N, L)
			S, I, R = Int[], Int[], Int[]
			...
			plot!(P[e], S, alpha=.2, label="", c="blue")
			plot!(P[e], I, alpha=.2, label="", c="red")
			plot!(P[e], R, alpha=.2, label="", c="green")
		end
	end
	plot(P[1], P[2])
end
...
Como pode ser visto, um gráfico com porcentagem máxima de infectados e forma geral semelhante ao da esquerda (que na lista passada usava taxas de infecção e recuperação >0.2 e cerca de 0.002, rexpectivamente) necessita de valores muito baixos de recuperação (0.00001) e relativamente altos de infecção (0.5); ou seja, um outbreak precisa de taxas muito diferentes daquelas da lista anterior. Isso se deve a modelagem distinta, e para simular uma mesma infecção usando os dois modelos (visando ter resultados semlehantes), acomodações à essas taxas devem ser feitas, nominalmente escalá-las por algum fator que é proporcional a N/L.
"""
